...
CreateOutputFile(const caas::FileContents &response) {
...
std::error_code openNativeFileInternalRedirectImpl(const llvm::Twine &Name,
  ...
  caas::OpenResponse r;
  g_msgpump->TransactOrDie(caas::OpenRequest{g_compileID, Name.str(), write, create}, r);
  switch (r.error) {
  case caas::FileError::AccessViolation:
    return std::make_error_code(std::errc::permission_denied);
  case caas::FileError::NotExist:
    return std::make_error_code(std::errc::no_such_file_or_directory);
  default:
    break;
  }

  ResultFile = (HANDLE)(size_t)r.contents.handle;
