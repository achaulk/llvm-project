...
public:
  VVFS_RemoteFile(caas::OpenResponse &&info)
      : name_(std::move(info.contents.canonical_filename)),
        h_((HANDLE)(size_t)info.contents.handle),
        offset_(info.contents.handle_offset), size_(info.contents.size),
        uuid_(info.contents.uuid) {
    if (!info.contents.contents_as_string.empty())
      contents_ = std::move(info.contents.contents_as_string);
  }
...
public:
  ...
  llvm::ErrorOr<Status> status(const Twine &Path) override {
    caas::StatResponse resp;
    g_msgpump->TransactOrDie(caas::StatRequest{g_compileID, Path.str()}, resp);

    if (!resp.exists)
      return llvm::ErrorOr<Status>(
          std::make_error_code(std::errc::no_such_file_or_directory));
    return Status(Path, llvm::sys::fs::UniqueID(1, resp.uuid),
                  sys::toTimePoint(0), 0, 0, resp.size,
                  resp.is_dir ? sys::fs::file_type::directory_file
  ...
  openFileForRead(const Twine &Path) override {
    ...
    g_msgpump->TransactOrDie(
        caas::OpenRequest{g_compileID, Path.str(), false, false}, resp);
    return std::make_unique<VVFS_RemoteFile>(std::move(resp));
  }
